...
export default class Histogram {
  ...
  width: number
  height: number
  constructor(table: FrequencyTable, width: number, height: number) {
  ...
  get xMin() : number {
    if (this.frequencyTable.rows.length === 0) return 0
    return this.frequencyTable.rows[0].lowerBound
  }
  
  get xMax(): number {
    if (this.frequencyTable.rows.length === 0) return 10
  ...
  get maxFD(): number {
    if (this.frequencyTable.rows.length === 0) return 1
    let maxFD = 0
    this.frequencyTable.rows.forEach( row => {
      if (row.frequencyDensity > maxFD) maxFD = row.frequencyDensity
    })
  ...
  scaledCoords([x,y]: [number,number]) : [number,number] {
